...
/**
 * Encoder for KML 2.2. This encoder uses only a streaming XML generator and should scale very well
...
/*
		writer.writeStartElement("IconStyle");
		writer.writeStartElement("color");
		writer.writeCharacters("ff509c98");
		writer.writeEndElement();
		writer.writeStartElement("colorMode");
		writer.writeCharacters("normal");
		writer.writeEndElement();
...
*/
	...
	private void writePolygon(XMLStreamWriter writer, Polygon p)
			throws XMLStreamException {
	...
	private void writeCoordinates(XMLStreamWriter writer, LineString ls)
			throws XMLStreamException {
		...
		for (int ic = 0; ic < coordinates.length; ic++) {
			coordString.append(coordinates[ic].x + "," + coordinates[ic].y
					+ ",0\n");
		}
	...
	private void writeLineString(XMLStreamWriter writer, LineString ls)
			throws XMLStreamException {
	...
	private void writeLinearRing(XMLStreamWriter writer, LineString lr)
			throws XMLStreamException {
	...
	private void writePoint(XMLStreamWriter writer, Point pt)
			throws XMLStreamException {
	...
	private String writeSchema(XMLStreamWriter writer,
			SimpleFeatureCollection fc) throws XMLStreamException {
		...
		for (PropertyDescriptor p : schema.getDescriptors()) {
			...
			if (!(p.getType() instanceof GeometryType)) {
				...
				String kmlType;
				/*
				 * KML has the following types ,
				 * https://developers.google.com/kml
				 * /documentation/kmlreference#simplefield string int uint short
	...
	private void writeExtendedData(XMLStreamWriter writer, SimpleFeature f,
			String schemaName) throws XMLStreamException {
